...
const OfficeHoliday = require('../models/OfficeHoliday');
...
const canManageOfficeHolidays = (user) => {
...
router.get('/', async (req, res) => {
  try {
    const items = await OfficeHoliday.find({ isActive: true }).sort({ date: 1, name: 1 }).lean();
...
router.post('/', auth, async (req, res) => {
  try {
    if (!canManageOfficeHolidays(req.user)) {
    ...
    const existing = await OfficeHoliday.findOne({
      name,
    ...
    const created = await OfficeHoliday.create({
      name,
      date: range.start,
      isActive: true,
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    const deleted = await OfficeHoliday.findByIdAndDelete(req.params.id);
