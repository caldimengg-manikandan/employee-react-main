...
const createPayrollWorkbook = (simulation, selectedMonth) => {
  ...
  const dataRows = simulation.results.map((record, index) => {
    ...
    // Use the calculated PF components from simulation/record
    // Employee PF contribution (what used to be calculated as 12% sum)
    const totalEmployeePF = record.employeePF || 0;
    
    // Split Employee PF into EPS and EPF for export if necessary, 
    // but the user's specific formula is just Employee PF = Total - 1950.
    // In many companies, the split is 8.33% and 3.67%.
    // We'll use the record's employeePF value.
    const epsDeduction = Math.round(totalEmployeePF * (8.33/12));
    const epfDeduction = totalEmployeePF - epsDeduction;
  ...
  const totals = simulation.results.reduce((acc, record, index) => {
    ...
    // EPF calculations
    const totalEmployeePF = record.employeePF || 0;
    const epsVal = Math.round(totalEmployeePF * (8.33/12));
    acc.eps += epsVal;
    acc.epf += (totalEmployeePF - epsVal);
...
export default function MonthlyPayroll() {
    ...
    try {
      ...
      const mapped = employees.map(emp => {
        ...
        return {
          ...
          designation: emp.designation || (activeComp ? activeComp.designation : '') || emp.position || payrollRec?.designation || '',
          department: emp.department || (activeComp ? activeComp.department : '') || emp.division || '',
  ...
  const runSimulation = async () => {
    ...
    try {
      // Fetch approved leaves overlapping with the selected month.
      // lopDays is already stored on each LeaveApplication record (set at approval time)
      // — this is the single source of truth, same as Leave Application History.
      ...
      const results = selectedRecords.map(rec => {
        ...
        // Get employee leaves for this month (only Approved, overlapping with selected month)
        const empId = String(rec.employeeId || '').trim();
        const employeeLeaves = leaves
          .filter(l => l.employeeId && String(l.employeeId).trim() === empId)
          .sort((a, b) => new Date(a.startDate) - new Date(b.startDate));

        // ─── LOP DAYS: Use the pre-calculated lopDays stored on each LeaveApplication ───
        // This is the SAME value shown in Leave Application History and is the single source
        ...
        let lopDaysInMonth = employeeLeaves
          .filter(l => {
            // Include any leave that overlaps this month
            const s = new Date(l.startDate);
            const e = new Date(l.endDate);
            return s <= monthEndLocal && e >= monthStartLocal;
          })
          .reduce((sum, l) => sum + (Number(l.lopDays) || 0), 0);

        // Total LOP days = leave LOP + Pre-DOJ days
