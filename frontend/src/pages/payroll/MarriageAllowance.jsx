...
const MarriageAllowance = () => {
  ...
  const renderList = () => (
    <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200/80 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2">
          <button onClick={()=>setShowFilters(prev=>!prev)} className="border border-slate-200 bg-slate-50/50 px-4 py-2 rounded-xl text-slate-700 font-semibold text-sm flex items-center gap-2 hover:bg-slate-100 transition-all shadow-sm">
            <Filter className="h-4 w-4 text-slate-500" /> Filters
          </button>
        </div>
        <div className="flex items-center gap-2.5">
          <button onClick={()=>{ setShowForm(true); resetForm(); }} className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 hover:from-slate-800 hover:to-indigo-900 transition-all shadow-sm">
            <Plus className="h-4 w-4" /> New Allowance
          </button>
          <button onClick={exportPDF} className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all shadow-sm">
      ...
      {showFilters && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-5 bg-indigo-50/70 p-4 rounded-xl border border-indigo-100 shadow-inner">
          <div className="relative">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
            <input
              className="pl-9 w-full border border-slate-200 bg-white rounded-xl p-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
          ...
          <div>
            <select className="w-full border border-slate-200 bg-white rounded-xl p-2 text-sm text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" value={filters.division} onChange={(e)=>setFilters(prev=>({...prev, division:e.target.value}))}>
              <option value="">All Divisions</option>
              {divisionOptions.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
          </div>
          <div>
            <select className="w-full border border-slate-200 bg-white rounded-xl p-2 text-sm text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" value={filters.location} onChange={(e)=>setFilters(prev=>({...prev, location:e.target.value}))}>
        ...
        <table className="min-w-full text-sm divide-y divide-slate-100">
          <thead>
            <tr className="text-left bg-[#262760] text-white">
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">S.no</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Employee ID</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Employee Name</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Division</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Location</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Marriage Date</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider">Payment Status</th>
              <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-slate-100">
            {filteredClaims.map((c, idx) => (
              <tr key={c._id} className="hover:bg-slate-50/80 transition-colors">
                <td className="px-6 py-4 font-semibold text-slate-500 text-sm whitespace-nowrap">{idx + 1}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-500 text-white flex items-center justify-center font-bold text-xs shadow-sm">
                      {(c.employeeName || 'E').charAt(0).toUpperCase()}
                    </div>
                    <div className="font-semibold text-slate-900 text-sm">{c.employeeName}</div>
                  </div>
                </td>
                <td className="px-6 py-4 text-sm font-medium text-slate-700 whitespace-nowrap">{c.division}</td>
                <td className="px-6 py-4 text-sm font-medium text-slate-600 whitespace-nowrap">{c.location}</td>
                <td className="px-6 py-4 text-sm text-slate-700 whitespace-nowrap">{c.marriageDate ? new Date(c.marriageDate).toLocaleDateString() : ''}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${c.paymentStatus === 'Paid' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                    {c.paymentStatus}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-1.5 justify-center">
                    <button className="p-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-lg transition-all shadow-sm" onClick={()=>setViewingItem(c)} title="View"><Eye className="h-4 w-4" /></button>
                    <button className="p-2 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg transition-all shadow-sm" onClick={()=>handleEdit(c)} title="Edit"><Edit2 className="h-4 w-4" /></button>
                    <button className="p-2 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-lg transition-all shadow-sm" onClick={()=>handleDelete(c)} title="Delete"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
            {filteredClaims.length === 0 && !loading && (
              <tr>
                <td className="p-6 text-center text-slate-500 font-medium" colSpan={8}>No marriage allowance records found</td>
              </tr>
            )}
          </tbody>
        </table>
